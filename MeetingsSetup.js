...
//rest api to get a particular Meeting data
app.get('/User/:idUser', function (req, res) {
   console.log(req);
   connection.query('select * from User where idUser=?', [req.params.idUser], function (error, results, fields) {
    if (error) throw error;
    res.send(results);
...
app.put('/User', function (req, res) {
   connection.query('UPDATE `User` SET `UserName`=?,`Emailid`=?,`FirstName`=? where `idUser`=?', [req.body.UserName,req.body.Emailid, req.body.FirstName, req.body.idUser], function (error, results, fields) {
    if (error) throw error;
    res.send(results);
